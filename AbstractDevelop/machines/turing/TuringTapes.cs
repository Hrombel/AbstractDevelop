...
namespace AbstractDevelop.machines.turing
{
    ...
    /// </summary>
    [Serializable]
    public class TuringTapes : ISerializable
    {
        /// <summary>
        ...
        public void SetValues(char[] vals)
        {
            ...
            for(int i = 0; i < n; i++)
            {
                if (vals[i] != _defaultChar)
                {
                    ...
                    _units[i].Tape.SetValue(_units[i].Position, (byte)(_symbols.GetCharCode(vals[i]) + 1));
                }
                else
                    _units[i].Tape.SetValue(_units[i].Position, 0);
            }
        }
        ...
        public char[] GetValues()
        {
            ...
            for (int i = 0; i < n; i++ )
            {
                code = _units[i].Tape.GetValue(_units[i].Position);
                if (code != 0)
