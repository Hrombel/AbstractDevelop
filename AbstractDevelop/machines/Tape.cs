...
using IndexType = System.Numerics.BigInteger;

namespace AbstractDevelop.Machines
{
    [Serializable]
    public class Tape<DataType> :
        IEnumerable<DataType>, ISerializable
    {
        /// <summary>
        ...
        public class TapeBlock :
            IEnumerable<DataType>, IComparable<TapeBlock>, ISerializable
        {
            #region [Поля]
            ...
            {
                info.AddValue("start", Start, typeof(IndexType));
                info.AddValue("data", container, typeof(DataType[]));
            }
            ...
            internal TapeBlock(IndexType start, int length = DefaultBlockSize, DataType value = default(DataType))
            {
                Start = start;
                container = Enumerable.Repeat(value, length).ToArray();
            }

            internal TapeBlock(SerializationInfo info, StreamingContext context)
            {
                Start = (info.GetValue("start", typeof(IndexType)) as IndexType?) ?? default(IndexType);
                container = (info.GetValue("data", typeof(DataType[])) as DataType[]) ?? new DataType[0];
            }
        ...
        public TapeBlock FindBlock(IndexType index, out int localIndex)
        {
            ...
            var value = Blocks.FirstOrDefault(block =>
            {
                ...
                return local.CheckIndex(false, max: block.Length);
            });
        ...
        public void GetObjectData(SerializationInfo info, StreamingContext context)
        {
            var count = 0;
            foreach (var block in Blocks)
                info.AddValue("block" + count, block, typeof(TapeBlock));
            info.AddValue("amount", count + 1);
        }
        ...
        private Tape(SerializationInfo info, StreamingContext context)
        {
            try
            {
                var count = info.GetInt32("amount");
                while (count > 0) AddBlock(info.GetValue("block" + (count - 1), typeof(TapeBlock)) as TapeBlock);
            }
            catch (Exception ex) { throw new ArgumentException(string.Format("Неверные параметры десериализации: \"{0}\"", ex.Message), ex); }
        }
