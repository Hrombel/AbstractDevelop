...
namespace AbstractDevelop.controls.visuals
{
    ...
    /// </summary>
    public partial class RiscRegistersVisualizer :
        UserControl
    {
        #region [События]
        ...
        public Func<int, string> FormatFunction { get; set; }
        ...
        private void DrawCells(BigInteger pos)
        {
            ...
            valFormat.Alignment = StringAlignment.Center;
            ...
            using (Graphics g = Graphics.FromImage(_canvas))
            {
                ...
                {
                    ind = c.ToString();
                    try { val = _registers != null ? FormatFunction(_registers[c] & ByteMask) : "0"; }
                    catch { return; }
        ...
        private void InfiniteRegistersVisualizer_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            switch (e.KeyCode)
            {
                case Keys.Add:
                    ...
                    {
                        if (_registers[_currentRegister] < byte.MaxValue)
                        {
                            _registers[_currentRegister] = (byte)(_registers[_currentRegister] + 1);
                            changed = true;
                        }
                    }
                    break;
                ...
                case Keys.Subtract:
                    ...
                    {
                        if (_registers[_currentRegister] > byte.MinValue)
                        {
                            _registers[_currentRegister] = (byte)(_registers[_currentRegister] - 1);
                            changed = true;
                        }
               ...
               case Keys.Delete:
                    ...
                    _registers[_currentRegister] = byte.MinValue;
                    changed = true;
                    break;
        ...
        private void InfiniteRegistersVisualizer_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (_currentRegister != -1)
            {
                ...
                {
                    string str = _registers[_currentRegister].ToString();
                    str += e.KeyChar;
                    _registers[_currentRegister] = (byte)(Math.Min(int.Parse(str), byte.MaxValue));
        ...
        public RiscRegistersVisualizer()
        {
            _velocity = 0;
            _pos = 0;
            _currentRegister = -1;
